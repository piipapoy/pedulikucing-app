...
import { 
  ...
  Dimensions, ActivityIndicator, StatusBar, Linking, Platform, Alert
} from 'react-native';
...
export default function ShelterDetail() {
  ...
  const handleChatShelter = async () => {
    try {
      ...
      router.push({
        pathname: '/chat-room',
        params: { roomId: res.data.id, name: shelter.nickname || shelter.name, avatar: resolveImageUrl(shelter.shelterPhotos) }
      });
    } catch (error) {
  ...
  const images = shelter.shelterPhotos ? shelter.shelterPhotos.split(',') : [];
      ...
      <View style={styles.headerContainer}>
        <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={styles.heroScroll}>
          {images.length > 0 ? images.map((img, i) => (
            <Image key={i} source={{ uri: resolveImageUrl(img) }} style={styles.heroImage} />
          )) : (
            <Image source={{ uri: 'https://via.placeholder.com/400' }} style={styles.heroImage} />
          )}
        </ScrollView>
        ...
        <View style={styles.heroContent}>
          <View style={styles.badgeRow}>
            <View style={styles.badgeShelter}>
              <Text style={styles.badgeText}>{shelter.isClinic ? 'Klinik & Shelter' : 'Shelter Resmi'}</Text>
            </View>
          </View>
                  ...
                  <View style={styles.cardInfo}>
                    ...
                    <View style={styles.miniTagWrapper}>
                      {cat.personality?.split(',').slice(0, 2).map((p, i) => (
                        <View key={i} style={styles.miniTagPerso}><Text style={styles.miniTagText}>{p.trim()}</Text></View>
                      ))}
                    </View>
          ...
          <View>
            {shelterCampaigns.length === 0 ? (
              ...
              shelterCampaigns.map(camp => (
                <TouchableOpacity key={camp.id} style={styles.campCard} activeOpacity={0.9} onPress={() => router.push({ pathname: '/donation-detail', params: { id: camp.id } })}>
                  <Image source={{ uri: camp.imageUrl }} style={styles.campImg} />
                  <View style={styles.campInfo}>
                    <Text style={styles.campTitle} numberOfLines={2}>{camp.title}</Text>
                    <View style={styles.progressBg}><View style={[styles.progressFill, { width: `${Math.min(100, (camp.currentAmount/camp.targetAmount)*100)}%` }]} /></View>
...
const styles = StyleSheet.create({
  ...
  badgeShelter: { backgroundColor: COLORS.secondary, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
